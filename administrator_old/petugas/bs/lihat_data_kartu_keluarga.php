...
<?php 
    $no_kartu_keluarga = $_GET['no_kartu_keluarga'];
       $sql_lihat_kartu = mysql_query("SELECT * from detail_kartu_keluarga d inner join kartu_keluarga kk on d.id_kk = kk.id_kk inner join penduduk p on d.no_kepala_keluarga = p.no_kepala_keluarga inner join dusun dus on p.iddusun = dus.iddusun inner join desa des on dus.iddesa = des.iddesa inner join kecamatan kec on des.idkecamatan = kec.idkecamatan inner join kabupaten kab on kec.idkabupaten = kab.idkabupaten inner join propinsi pro on kab.idpropinsi = pro.idpropinsi where no_kartu_keluarga = '$no_kartu_keluarga' 
        ");
       $tampilheader = mysql_fetch_array($sql_lihat_kartu);
       $sql_lihat_kartu2 = mysql_query("SELECT * from detail_kartu_keluarga d inner join kartu_keluarga kk on d.id_kk = kk.id_kk inner join penduduk p on d.no_kepala_keluarga = p.no_kepala_keluarga inner join dusun dus on p.iddusun = dus.iddusun inner join desa des on dus.iddesa = des.iddesa inner join kecamatan kec on des.idkecamatan = kec.idkecamatan inner join kabupaten kab on kec.idkabupaten = kab.idkabupaten inner join propinsi pro on kab.idpropinsi = pro.idpropinsi where no_kartu_keluarga = '$no_kartu_keluarga' 
        ");
            ...
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Daftar Kartu Keluarga 
                       <!--  <a href="print_kartu_keluarga.php&no_kartu_keluarga=<?php echo $tampilheader['no_kartu_keluarga']; ?>" target="_blank" class="btn btn-warning">Cetak KK</a> -->
                    </h1>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="panel panel-primary">
                        ...
                        <?php 
                            $no = 1;
                            while ($kolom = mysql_fetch_array($sql_lihat_kartu)) {
